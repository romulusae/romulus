...
def skinny_enc(plaintext, tweakey, version):
    ...
    for i in range(TAB_ROUNDS[version]-1):
        ...
        for j in range(8): 
            ...
            elif version in [4,5,6]: tk[i+1][j+16] = ((tk[i+1][j+16]<<1)&0xfe) ^ (((tk[i+1][j+16]>>7) ^ (tk[i+1][j+16]>>5))&0x1)
            if version in [2]: tk[i+1][j+32] = (tk[i+1][j+32]>>1) ^ (((tk[i+1][j+32]<<3) ^ (tk[i+1][j+32]))&0x8)
            elif version in [5,6]: tk[i+1][j+32] = (tk[i+1][j+32]>>1) ^ (((tk[i+1][j+32]<<7) ^ (tk[i+1][j+32]<<1))&0x80)
